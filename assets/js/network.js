(function () {
  ...
  function curate(nodes, edges, limit) {
    ...
    return {
      nodes: nodes.filter(n => keep.has(n.pubkey)),
  ...
  function render(graph) {
    ...
    const r = d => isHero
      ? 3   + Math.min(7, (d.neighbor_count || 1) * 0.5)
    ...
    const sim = d3.forceSimulation(graph.nodes)
      .force("link",    d3.forceLink(graph.edges).id(d => d.pubkey)
        .distance(() => isHero ? 50 : 36)
        .strength(d  => 0.05 + 0.4 * (d.score || 0)))
    ...
    if (!isHero) {
      svg.call(
        d3.zoom().scaleExtent([0.3, 6]).on("zoom", ev => g.attr("transform", ev.transform))
      );
      node.call(
        d3.drag()
          .on("start", (ev, d) => {
  ...
  function refreshStats() {
    loadStats()
      ...
      .catch(() => { /* silent — keep interval alive */ });
  }
  ...
  load().then(data => {
    if (!data) {
      mount.innerHTML = '<div class="net-err">network data unavailable</div>';
      return;
    }
